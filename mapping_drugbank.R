all <- read.csv("./drugbank_all_target_polypeptide_ids.fasta/all.fasta", header= TRUE, stringsAsFactors=FALSE)
nutraceutical <- read.csv("./drugbank_nutraceutical_target_polypeptide_ids.fasta/all.fasta",header= TRUE,stringsAsFactors=FALSE)

target.list <- read.csv("./target.list.csv", header= TRUE, stringsAsFactors=FALSE)
...
target.list$DrugBank <-(target.list$Identifier %in% all[all$No.Cofactor,]$UniProt.ID)
